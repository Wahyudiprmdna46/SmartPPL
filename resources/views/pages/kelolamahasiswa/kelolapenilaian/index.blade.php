...
@section('content')
    <div class="jumbotron mt-3">
        ...
        <p>Pengelolaan penilaian mahasiswa ppl dari Pamong</p>
    </div>
    ...
    <div class="row">
                                ...
                                @forelse($penilaians as $item)
                                    <tr style="border: 2px solid #2c3e50; background-color: white;">
                                        <td class="text-center">
                                            {{ $loop->iteration }}</td>
                                        <td class="text-center">
                                            {{ $item->dataMahasiswa->nim }}</td>
                                        <td class="text-center">
                                            {{ $item->dataMahasiswa->nama }}</td>
                                        <td class="text-center">
                                            ...
                                            <div class="d-flex justify-content-center gap-2">
                                                @if ($canEdit)
                                                    <button class="btn btn-sm btn-warning btn-edit mr-2"
                                                        ...
                                                        data-id="{{ $item->id }}"
                                                        data-mahasiswa="{{ $item->dataMahasiswa->nama }}"
                                                ...
                                                @if ($canExport)
                                                    <a href="{{ route('penilaianpdf', $item->id) }}" class="btn btn-sm btn-success mr-2">
                                                        <i class="fas fa-download"></i>
                                                    </a>
                                                    <a href="{{ route('penilaianpdfshow', $item->id) }}" class="btn btn-sm btn-primary" target="_blank">
        ...
        @include('pages.kelolamahasiswa.kelolapenilaian._modal-create', ['mahasiswas' => $mahasiswas])

        {{-- Modal Edit Penilaian --}}
        @include('pages.kelolamahasiswa.kelolapenilaian._modal-edit')
    @endsection
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        // Set action ke form
                        formEdit.action = '{{ route('penilaian.update', ':id') }}'.replace(':id', id);
                    });
                });
            });
        </script>
