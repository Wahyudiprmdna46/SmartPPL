...
@else
    <div class="d-flex justify-content-end mb-3 gap-2 text-right">
        <form action="{{ route('downloadpdfsekolah') }}" method="POST" target="_blank" class="mr-2">
            @csrf
        ...
        <form action="{{ route('showpdfsekolah') }}" method="POST" target="_blank">
            @csrf
            <input type="hidden" name="bulan" value="{{ $bulan }}">
            <input type="hidden" name="tahun" value="{{ $tahun }}">
