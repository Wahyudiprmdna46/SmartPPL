    ...
    <div class="container mt-4">
        ...
        <div class="card mt-3 shadow-lg">
            <div class="card-header d-flex justify-content-between align-items-center bg-white">
                {{-- Search Form --}}
                <form method="GET" action="{{ route('datapamong') }}" class="d-flex align-items-center gap-2">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" placeholder="Cari Nama atau NIP..." value="{{ request('search') }}">
                        
                        @if(request('search'))
                            <a href="{{ route('datapamong') }}" class="btn btn-outline-secondary">
                                &times;
                            </a>
                        @endif
                ...
                <div>
                    ...
                    <div class="modal fade" id="importExcelModal" tabindex="-1" aria-labelledby="importExcelModalLabel"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('importexcelpamong') }}" method="POST"
                    ...
                    <a href="{{ route('datapamongcreate') }}" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Tambah Data
                    </a>

                    <a href="{{ route('exportexcelpamong') }}" class="btn btn-success">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table class="table-striped table-hover table align-middle"
                        ...
                        <tbody>
                            @foreach ($datapamongs as $index => $datapamong)
                                <tr style="border: 2px solid #2c3e50; background-color: white;">
                                    <td class="text-center">
                                        {{ $index + $datapamongs->firstItem() }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->nip }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->nama }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->jenis_kelamin ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->golongan ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->jabatan ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datapamong->dataSekolah?->nama_sekolah ?? '-' }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('viewmahasiswapamong', ['id' => $datapamong->id]) }}"
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex">
                                            <a href="{{ route('datapamongedit', $datapamong->id) }}"
                                                class="btn btn-sm btn-warning mr-2">
                                                Ubah
                                            </a>
                                            <form action="{{ route('datapamongdelete', $datapamong->id) }}" method="POST"
