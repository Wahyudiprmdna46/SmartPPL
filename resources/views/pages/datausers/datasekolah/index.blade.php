    ...
    <div class="container mt-4">
        ...
        <div class="card mt-3 shadow-lg">
            <div class="card-header d-flex justify-content-between align-items-center bg-white">
                {{-- Search Form --}}
                <form method="GET" action="{{ route('datasekolah') }}" class="d-flex align-items-center gap-2">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" placeholder="Cari Nama atau NIP..." value="{{ request('search') }}">
                        
                        @if(request('search'))
                            <a href="{{ route('datasekolah') }}" class="btn btn-outline-secondary">
                                &times;
                            </a>
                        @endif
                ...
                <div>
                    ...
                    <div class="modal fade" id="importExcelModal" tabindex="-1" aria-labelledby="importExcelModalLabel"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('importexcelsekolah') }}" method="POST"
                    ...
                    <a href="{{ route('datasekolahcreate') }}" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Tambah Data
                    </a>
                    
                    <a href="{{ route('exportexcelsekolah') }}" class="btn btn-success">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table class="table-striped table-hover table align-middle"
                        ...
                        <tbody>
                            @foreach ($datasekolahs as $index => $datasekolah)
                                <tr style="border: 2px solid #2c3e50; background-color: white;">
                                    <td class="text-center">
                                        {{ $index + $datasekolahs->firstItem() }}</td>
                                    <td class="text-center">
                                        {{ $datasekolah->npsn }}</td>
                                    <td class="text-center">
                                        {{ $datasekolah->nama_sekolah }}</td>
                                    <td class="text-center">
                                        {{ $datasekolah->dataDpl?->nama ?? '-' }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('viewmahasiswasekolah', ['id' => $datasekolah->id]) }}"
                                            class="btn btn-sm btn-primary mr-2">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        {{ $datasekolah->alamat }}</td>
                                    <td class="text-center">
                                        {{ $datasekolah->kota }}</td>
                                    <td class="text-center">
                                        {{ $datasekolah->provinsi }}</td>
                                    <td class="text-center">
                                        <div class="d-flex">
                                            <a href="{{ route('datasekolahedit', $datasekolah->id) }}"
                                                class="btn btn-sm btn-warning mr-2">
                                                Ubah
                                            </a>
                                            <form action="{{ route('datasekolahdelete', $datasekolah->id) }}"
                                                method="POST"
