    ...
    <div class="container mt-4">
        ...
        <div class="card mt-3 shadow-lg">
            <div class="card-header d-flex justify-content-between align-items-center bg-white">
                {{-- Search Form --}}
                <form method="GET" action="{{ route('datamahasiswa') }}" class="d-flex align-items-center gap-2">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" placeholder="Cari Nama atau NIM..."
                            value="{{ request('search') }}">

                        @if (request('search'))
                            <a href="{{ route('datamahasiswa') }}" class="btn btn-outline-secondary">
                                &times;
                            </a>
                        @endif
                ...
                <div>
                    ...
                    <div class="modal fade" id="importExcelModal" tabindex="-1" aria-labelledby="importExcelModalLabel"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('importexcelmahasiswa') }}" method="POST"
                    ...
                    <a href="{{ route('datamahasiswacreate') }}" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Tambah Data
                    </a>

                    <a href="{{ route('exportexcelmahasiswa') }}" class="btn btn-success">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table class="table-striped table-hover table align-middle"
                        ...
                        <tbody>
                            @foreach ($datamahasiswas as $index => $datamahasiswa)
                                <tr style="border: 2px solid #2c3e50; background-color: white;">
                                    <td class="text-center">
                                        {{ $index + $datamahasiswas->firstItem() }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->nim }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->nama }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->jenis_kelamin ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->jurusan }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->dataDpl?->nama ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->dataSekolah?->nama_sekolah ?? '-' }}</td>
                                    <td class="text-center">
                                        {{ $datamahasiswa->dataPamong?->nama ?? '-' }}</td>
                                    <td class="text-center">
                                        <div class="d-flex">
                                            <a href="{{ route('datamahasiswaedit', $datamahasiswa->id) }}"
                                                class="btn btn-sm btn-warning mr-2">
                                                Ubah
                                            </a>
                                            <form action="{{ route('datamahasiswadelete', $datamahasiswa->id) }}"
                                                method="POST"
